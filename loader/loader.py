...
# def get_MMD_loader(source_path,labeled_target_path,unlabeled_target_path,batch_size=8):
#     cfg=config.cfg
#     setup_seed()
#     val_set=eval(cfg['loader']['name']+'_Dataset')(cfg,dir=source_path,
#                                                         is_train=False)
...
def get_MMD_loader(unlabeled_target_path,batch_size=8):
    cfg=config.cfg
    setup_seed()
    val_set=eval(cfg['loader']['name']+'_Dataset')(cfg,dir=unlabeled_target_path,
                                                        is_train=False)
    dataloaders=DataLoader(val_set, batch_size=batch_size,
                                 shuffle=cfg['loader']['shuffle'], num_workers=cfg['loader']['num_workers'],worker_init_fn=_init_fn)
    return dataloaders   
        ...
        if mode=='RandomDel':
            ...
            if self.is_train:
                ...
                if l>3:
                    ...
                    path_post+=[None]*(16-len(path_post))
                    labels+=[None]*(16-len(labels))
                else:
                    path_post=path_post_
                    labels=labels_                    
                # print(len(labels))
            ...
            if self.is_train:
                ...
                if l<16-l:
                    ...
                    copy_num=random.randint(0,16-l)
                if l+copy_num-5>0:
                    del_num=random.randint(0,l+copy_num-5)#最少留3个
                else:
                    del_num=0
