...
class mynet12(nn.Module):
    ...
    def __init__(self, layers=[1, 1, 1, 1], n_class=2, model_path=None):
        ...
        self.stack1 = self.make_stack(64, 1)
        self.stack2 = self.make_stack(128, 1, stride=1)
        self.se=SELayer(512)
...
# %%
class mynet12mtc1v2(nn.Module):
    def __init__(self,encoder=mynet12,planes=128,max_len=16,n_class=2):
        super(mynet12mtc1v2,self).__init__()
        self.planes=planes
        ...
        self.hand=nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Conv1d(in_channels=128,out_channels = n_class, kernel_size = 3,padding=1),
            nn.ReLU(inplace=True)
        )
        # self.init_param()
