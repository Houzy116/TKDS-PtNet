...
class mynet2pre(nn.Module):
    """
    block: A sub module
    """

    def __init__(self,n_class=2):
        super(mynet2pre, self).__init__()
        self.inplanes = 16
        # self.modelPath = model_path
        self.encoder=Encoder()
        self.pos_embedding = nn.Parameter(torch.randn(1, 128, 50))
        ...
        self.fn2=nn.Linear(200,n_class)
...
class Encoder(nn.Module):
    def __init__(self):
        ...
        self.conv2=nn.Sequential(nn.Conv2d(6, 16, kernel_size=1, stride=1,bias=False),
                                nn.BatchNorm2d(16),
                                nn.Conv2d(16, 64, kernel_size=2, stride=2,bias=False),
                                nn.BatchNorm2d(64),
                                nn.Conv2d(64, 128, kernel_size=1, stride=1,bias=False),
                                nn.BatchNorm2d(128),
                                nn.ReLU(inplace=True),
        ...
        self.conv3=nn.Sequential(nn.Conv2d(6, 16, kernel_size=1, stride=1,bias=False),
                                nn.BatchNorm2d(16),
                                nn.Conv2d(16, 64, kernel_size=3, stride=3,bias=False),
                                nn.BatchNorm2d(64),
                                nn.Conv2d(64, 128, kernel_size=1, stride=1,bias=False),
                                nn.BatchNorm2d(128),
                                nn.ReLU(inplace=True),
    ...
    def forward(self,x):
        ...
        # print(x3.shape)
        # print(x6.shape)
        out=torch.cat([x1,x2,x3,x6],dim=-1)
        return out
