...
# %%
class resnet9v2mt(nn.Module):
    def __init__(self,encoder=resnet9v2,planes=128,max_len=16,n_class=2):
        super(resnet9v2mt,self).__init__()
        self.planes=planes
        self.n_class=n_class
        self.max_len=max_len
        self.encoder=encoder(planes=planes)
        self.head = nn.Sequential(
                nn.Linear(planes*max_len, planes*max_len),
                nn.ReLU(inplace=True),
                nn.Linear(planes*max_len, planes*max_len),
                nn.ReLU(inplace=True),
                nn.Linear(planes*max_len, max_len*n_class),                
            )
        self.init_param()
