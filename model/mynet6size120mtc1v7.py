...
class mynet6size120(nn.Module):
    ...
    def __init__(self, layers=[2, 2, 1, 1], n_class=2, model_path=None):
        ...
        self.fn1=nn.Linear(128,32)
        self.ln=nn.LayerNorm(36*32)
...
# %%
class mynet6size120mtc1v7(nn.Module):
    def __init__(self,encoder=mynet6size120,planes=128,max_len=16,n_class=2):
        super(mynet6size120mtc1v7,self).__init__()
        self.planes=planes
        ...
        self.hand=nn.Sequential(
            nn.Conv1d(in_channels=36*32,out_channels = 36*32, kernel_size = 3,padding=1),
            nn.ReLU(inplace=True),
            nn.Conv1d(in_channels=36*32,out_channels = n_class, kernel_size = 3,padding=1),
            nn.ReLU(inplace=True)
        )
        # self.init_param()
    ...
    def forward(self,x):
        ...
        # print(x.shape)
        x=x.view(B,L,36*32).permute(0, 2, 1)#B,L,C
        # print(x.shape)
        x=self.hand(x)
        x=x.permute(0,2,1)
        return x
