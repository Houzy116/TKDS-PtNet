...
class mynet6size120(nn.Module):
    ...
    def __init__(self, layers=[2, 2, 1, 1], n_class=2, model_path=None):
        ...
        self.se=SELayer(512)
        self.fn=nn.Conv2d(512, 256, kernel_size=1, stride=1,
                        bias=False)
        # self.pos_embedding = nn.Parameter(torch.randn(1, 128, 36))
        # self.token=Semantic_tokens(channel=128,token_len=32)
        self.trf1=Transformer(256,1,8,256,256,0)
        self.trf2=Transformer(256,1,8,256,256,0)

        self.fn1=nn.Linear(256,16)
        # self.ln=nn.LayerNorm(144*4)
        # self.fn2=nn.Linear(144*4,144)
        # self.relu=nn.ReLU(inplace=True)
...
# %%
class mynet6size120mtc1v8(nn.Module):
    def __init__(self,encoder=mynet6size120,planes=128,max_len=16,n_class=2):
        super(mynet6size120mtc1v8,self).__init__()
        self.planes=planes
        ...
        self.hand=nn.Sequential(
            nn.Conv1d(in_channels=144*4,out_channels = 144*4, kernel_size = 3,padding=1),
            nn.ReLU(inplace=True),
            nn.Conv1d(in_channels=144*4,out_channels = n_class, kernel_size = 3,padding=1),
            nn.ReLU(inplace=True)
        )
        # self.init_param()
    ...
    def forward(self,x):
        ...
        x=x.view(-1,C,W,H)
        x=self.encoder(x)
        # x,output1,output2=self.encoder(x)
        # print(x.shape)
        x=x.view(B,L,144*4).permute(0, 2, 1)#B,L,C
        # print(x.shape)
        x=self.hand(x)
        x=x.permute(0,2,1)
        return x
