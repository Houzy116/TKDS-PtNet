...
class myseg6size120(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv_seg = nn.Conv2d(512, 128, kernel_size=1)
    def forward(self, x):
        x=self.mynet(x)
        x=self.uperhead(x)
        x=self.dropout(x)
...
class myseg6size120mtc1v2(nn.Module):
    def __init__(self,encoder=myseg6size120,planes=128,max_len=16,n_class=2):
        super(myseg6size120mtc1v2,self).__init__()
        self.planes=planes
        ...
        # self.init_param()
        self.patch_embadding=nn.Conv2d(128,256,kernel_size=6,stride=6)
        self.patch_embadding_transpose=nn.ConvTranspose2d(256, 128, kernel_size=6, stride=6)
    ...
    def init_param(self):
        ...
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                ...
                m.bias.data.zero_()
    def forward(self,x):
        B,L,W,H=x.shape[0],int(x.shape[1]/7),x.shape[2],x.shape[3]
        x = rearrange(x, 'b (l c) w h -> (b l) c w h ', c=7)
        x=self.encoder(x)
        input=x.detach()
        x=self.patch_embadding(x)
        x = rearrange(x, '(b l) c pw ph -> (b pw ph) c l ', l=L)

        x=self.hand(x)#B*W/4*H/4,C,L
        x = rearrange(x, '(b pw ph) c l -> (b l) c pw ph ', l=L, pw=int(W/6), ph=int(H/6))
        x=self.patch_embadding_transpose(x)
        out=torch.cat([input,x], dim=1)
        out=self.conv(out)
        out=rearrange(out,'(b l) c w h -> b l c w h',l=L)
        return out
...
# print(output.shape)
# #%%
# input=torch.rand(2,63,120,120).cuda()
# m=myseg6size120mtc1v2().cuda()
# output=m(input)
# print(output.shape)
# output.shape
# for i in range(4):
