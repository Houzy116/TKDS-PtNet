...
class USAmynet6size120(nn.Module):
    """
    block: A sub module
    """

    def __init__(self, layers=[2, 2, 1, 1], n_class=2, model_path=None):
        super(USAmynet6size120, self).__init__()
        self.inplanes = 64
...
# %%
class USAmynet6size120mtc1v2v4(nn.Module):
    def __init__(self,encoder=USAmynet6size120,planes=128,max_len=12,n_class=1):
        super(USAmynet6size120mtc1v2v4,self).__init__()
        self.planes=planes
        ...
        self.hand=nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Conv1d(in_channels=144,out_channels = 2, kernel_size = 3,padding=1),
            nn.ReLU(inplace=True)
        )
        # self.init_param()
    ...
    def forward(self,x):
        B,L,W,H=x.shape
        x=x.view(B,int(L/7),-1,W,H)
        B,L,C,W,H=x.shape
        x=x.view(-1,C,W,H)
        x=self.encoder(x)
        # print(x.shape)
        x=x.view(B,L,144).permute(0, 2, 1)#B,L,C
        # print(x.shape)
        x=self.hand(x)
        x=x.permute(0,2,1)
        return x
