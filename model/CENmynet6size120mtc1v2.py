...
class merge_head(nn.Module):
    def __init__(self):
        ...
        self.t_conv = nn.ConvTranspose2d(6,6,2,2,bias=False)
        self.bn = nn.BatchNorm2d(6)
        self.relu = nn.ReLU(inplace=True)
        self.conv1=nn.Conv2d(6,6,1,1,bias=False)
    ...
    def forward(self,input):
        ...
        return x 
class CENmynet6size120(nn.Module):
    """
    block: A sub module
    """

    def __init__(self, layers=[2, 2, 1, 1], n_class=2, model_path=None):
        super(CENmynet6size120, self).__init__()
        self.inplanes = 64
        self.modelPath = model_path
        self.merge_head=merge_head()
        self.conv1 = nn.Conv2d(10, 64, kernel_size=1, stride=1,
                               bias=False)
...
class CENmynet6size120mtc1v2(nn.Module):
    def __init__(self,encoder=CENmynet6size120,planes=128,max_len=5,n_class=1):
        super(CENmynet6size120mtc1v2,self).__init__()
        self.planes=planes
        ...
        self.hand=nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Conv1d(in_channels=144,out_channels = 3, kernel_size = 3,padding=1),
            nn.ReLU(inplace=True)
        )
        # self.init_param()
    ...
    def forward(self,x):
        ...
        x10=x10.view(B,int(L10/4),-1,W10,H10)
        x20=x20.view(B,int(L20/6),-1,W20,H20)


        x10=x10.view(-1,4,W10,H10)
        x20=x20.view(-1,6,W20,H20)
...
# %%
# input=[torch.rand(15,20,120,120),torch.rand(15,30,60,60)]
